'''1. Начать реализацию класса «Хранилище» для серверной стороны. Хранение
необходимо осуществлять в базе данных. В качестве СУБД использовать sqlite.
...
class ServerDB:
    ...
    def __init__(self):
        ...
        self.session.query(
            self.ActiveUsers).delete()  # при создании сессии удаляем старые двнные об активных пользователях
        self.session.commit()
